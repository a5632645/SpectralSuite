...
public:
    ParameterContainerComponent * createUi(SpectralAudioPlugin *plugin) override {
        return new SliderContainer(m_baseParams, Colour::fromString(TEXT_COLOUR), 30);
    };
    
    std::shared_ptr<PluginParameters> createParams(SpectralAudioPlugin *plugin) override {
        m_baseParams = std::make_shared<PDParameters>(plugin);
        return m_baseParams;
    };
    
    std::unique_ptr<SpectralAudioProcessorInteractor> createProcessor(SpectralAudioPlugin *plugin) override {
        return std::make_unique<PDInteractor>(SpectralAudioPlugin::FFT_OVERLAPS, m_baseParams);
    }
private:
    std::shared_ptr<PDParameters> m_baseParams;
};
