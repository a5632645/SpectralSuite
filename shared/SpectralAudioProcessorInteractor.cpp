...
SimpleBusLayout SpectralAudioProcessorInteractor::getProcessorBusWant() const {
    SimpleBusLayout l;
    l.channelPerInputBus = 2;
    l.inputBusCount = 1;
    l.outputBusChannelCount = 2;
    return l;
}

void SpectralAudioProcessorInteractor::spectralProcess(std::vector<BusPolarPlusData>& input, BusPolarPlusData& output) {
    auto& ch0 = input[0];
    std::copy(ch0[0].polar.cbegin(), ch0[0].polar.cend(), output[0].polar.begin());
    std::copy(ch0[1].polar.cbegin(), ch0[1].polar.cend(), output[1].polar.begin());
}